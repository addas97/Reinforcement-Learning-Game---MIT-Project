...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if p < epsilon:
        ...
        index = np.argmax(theta @ state_vector) # Q-values
        action_index, object_index = tuple2index(index)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    # Update theta - new theta = old theta + learning Q
    theta[tuple2index(action_index, object_index)] += ALPHA * (y - q_value) * current_state_vector
